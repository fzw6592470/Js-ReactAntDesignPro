...
for(let i=0; i< 88; i++) {
  let amountFee = Number(Math.floor(Math.random() * 1000));
  let paidedFee = Number(Math.floor(Math.random() * 100));
  let cancelFee = amountFee - paidedFee - i;
  transRecrod.push({
    key: i,
    disabled: false,
    wasteNo: `wasteNo_${i}`,
    transTime: moment(new Date( new Date().getTime() - 1000* 60* 60*i)).format('YYYY-MM-DD HH:mm:ss'),
  ...
  transDetails.push({
    ...
    payTime: moment(new Date( new Date().getTime() - 1000* 60* 60*i)).format('YYYY-MM-DD HH:mm:ss'),
    payType: i%5,
    orderTime: moment(new Date( new Date().getTime() - 1000* 60* 60*i)).format('YYYY-MM-DD HH:mm:ss'),
  });
}
...
export function getTransDetails(req, res, u) {
  ...
  if(params.date) {
    dataSource = dataSource.filter( data => data.transTime > params.date[0] && data.transTime < params.date[1] );
  }
